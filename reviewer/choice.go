...
var choiceAnswerReviewers = map[types.QuestionType]func(question *choice.Choice, answers []string) error{
...
func ReviewChoice(questionValue any, answers []string, qt types.QuestionType) error {
	if len(answers) == 0 {
		return nil
	}
...
func reviewCheckbox(questionValue *choice.Choice, answers []string) error {
	return choiceContainsAllAnswers(questionValue, answers)
}

// reviewSingleSelect validates the answers for a single select type.
func reviewSingleSelect(questionValue *choice.Choice, answers []string) error {
	if len(answers) > 1 {
...
func reviewMultipleSelect(questionValue *choice.Choice, answers []string) error {
	return choiceContainsAllAnswers(questionValue, answers)
}

// reviewRadio validates the answers for a radio type.
func reviewRadio(questionValue *choice.Choice, answers []string) error {
	if len(answers) > 1 {
...
func choiceContainsAllAnswers(questionValue *choice.Choice, answers []string) error {
